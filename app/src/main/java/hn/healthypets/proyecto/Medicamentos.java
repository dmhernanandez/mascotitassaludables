...
public class Medicamentos extends AppCompatActivity {
            ...
            if (fieldsFill && spiDosis.getSelectedItemPosition() > 0) {
                switch (accion) {
                    ...
                    case Constantes.ACTUALIZAR:
                        ...
                        Toast.makeText(Medicamentos.this, "Información con exito +", Toast.LENGTH_LONG).show();
                        finish();
                        break;
                }
            } else {
    ...
    private void init() {
        ...
        if (accion == Constantes.GUARDAR) {
            ...
            nombreMedicamento = instanciaDB.getTipoDosisDAO().getNameDoseTypeById(intentValues.getIntExtra(Constantes.TAG_ID, Constantes.DEFAULT));
            Log.i("prueba", "" + intentValues.getIntExtra(Constantes.TAG_ID, Constantes.DEFAULT));
            String[] fecha = intentValues.getStringExtra(Constantes.TAG_FECHA_APLICACION).split("-");
            dia = Integer.parseInt(fecha[0]);
            mes = Integer.parseInt(fecha[1]);
            anio = Integer.parseInt(fecha[2]);
            edtNombreMedicamento.setText(intentValues.getStringExtra(Constantes.TAG_NOMBRE));
            edtCadaDosis.setText(intentValues.getStringExtra(Constantes.TAG_CADA));
            edtPorDosis.setText(intentValues.getStringExtra(Constantes.TAG_POR));
            edtFechaMedicamento.setText(intentValues.getStringExtra(Constantes.TAG_FECHA_APLICACION));
